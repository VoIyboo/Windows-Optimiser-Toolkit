...
# Optional imports use the diagnostic helper so failures get logged rather
# than swallowed. Apps.Helpers is critical for this module so it stays Stop.
. (Join-Path $PSScriptRoot "..\Core\QOTImportHelper.ps1")
$null = Import-QOTOptionalModule -Path (Join-Path $PSScriptRoot "..\Core\Actions\ActionRegistry.psm1") -ImporterContext 'Apps.UI' -Global
...
function Initialize-QOTAppsUI {
    ...
    try {
        ...
        $null = Import-QOTOptionalModule -Path (Join-Path $PSScriptRoot "InstalledApps.psm1")     -ImporterContext 'Apps.UI.Initialize' -Force
        $null = Import-QOTOptionalModule -Path (Join-Path $PSScriptRoot "InstallCommonApps.psm1") -ImporterContext 'Apps.UI.Initialize' -Force
        ...
        Register-QOTActionGroup -Name "Apps" -GetItems ({
            ...
            if ($Window -is [System.Windows.FrameworkElement]) {
                try { $appsGrid = $Window.FindName("AppsGrid") } catch { }
                try { $installGrid = $Window.FindName("InstallGrid") } catch { }
            }
            ...
            $items += [pscustomobject]@{
                ...
                IsSelected = ({
                    ...
                    $appsGridCurrent = $appsGrid
                    $installGridCurrent = $installGrid
                    if ($window -is [System.Windows.FrameworkElement]) {
                        try {
                            $resolvedAppsGrid = $window.FindName("AppsGrid")
                            if ($resolvedAppsGrid) { $appsGridCurrent = $resolvedAppsGrid }
                        } catch { }
                        try {
                            $resolvedInstallGrid = $window.FindName("InstallGrid")
                            if ($resolvedInstallGrid) { $installGridCurrent = $resolvedInstallGrid }
                        } catch { }
                    }

                    $apps = if ($appsGridCurrent) { @($appsGridCurrent.ItemsSource) } else { @() }
                    $common = if ($installGridCurrent) { @($installGridCurrent.ItemsSource) } else { @() }
...
function Initialize-QOTCommonAppsCatalogue {
    ...
    foreach ($item in $catalogue) {
        Ensure-QOTCommonAppForGrid -App $item
