...
$null = Import-QOTOptionalModule -Path (Join-Path $PSScriptRoot "..\Logging\Logging.psm1") -ImporterContext 'ActionRegistry' -Force

$script:QOT_ActionGroups = $null
...
function Initialize-QOTActionGroups {
    if (-not $global:QOT_ActionGroups -or -not ($global:QOT_ActionGroups -is [System.Collections.Generic.List[object]])) {
        $global:QOT_ActionGroups = New-Object System.Collections.Generic.List[object]
    }

    if (-not $script:QOT_ActionGroups -or -not ($script:QOT_ActionGroups -is [System.Collections.Generic.List[object]]) -or -not [object]::ReferenceEquals($script:QOT_ActionGroups, $global:QOT_ActionGroups)) {
        $script:QOT_ActionGroups = $global:QOT_ActionGroups
    }
...
function Write-QOTActionConsoleLog {
    ...
    try {
        if (Get-Command Write-QLog -ErrorAction SilentlyContinue) {
            $level = if ($Status -eq "ERROR") { "ERROR" } elseif ($Status -eq "WARN") { "WARN" } else { "INFO" }
            Write-QLog $entry $level
            return
        }
    } catch { }
}
...
function Clear-QOTActionGroups {
    $groups = Initialize-QOTActionGroups
    $groups.Clear()
}
...
function Register-QOTActionGroup {
    ...
    [void](Initialize-QOTActionGroups)
...
function Get-QOTSelectedActionsInExecutionOrder {
    ...
    $indexedItems = New-Object System.Collections.Generic.List[object]
    for ($i = 0; $i -lt $selectedItems.Count; $i++) {
        $entry = $selectedItems[$i]
        if (-not $entry) { continue }

        $indexedItems.Add([pscustomobject]@{
            Entry = $entry
            SelectionOrder = $i
        }) | Out-Null
    }
    ...
    $ordered = @($indexedItems | Sort-Object @{ Expression = {
        if ($groupOrder.ContainsKey($_.Entry.Group)) { $groupOrder[$_.Entry.Group] } else { 999 }
    } }, @{ Expression = { $_.SelectionOrder } })
